// Taxonomie de la bibliothèque : catégories, familles, libellés.
//
// À part des données de recettes, et c'est délibéré : `FilterPanel` et la
// sidebar ont besoin de ces tables, pas des 3 896 recettes. Les garder dans le
// même module ferait repartir tout le corpus dans le bundle client.

const BISCUITS_VOYAGE = new Set([
  'cake', 'biscuit_moelleux', 'brownie', 'madeleine', 'cake_voyage', 'mi_cuit',
]);

export function getFamille(categorie, sous_categorie) {
  switch (categorie) {
    case 'glaces_sorbets': return 'glacerie';
    // `assemblages` décrit un dessert étage par étage (champ `composants`,
    // procédé de chaque préparation). `classiques` tient les mêmes desserts
    // vus autrement : une liste d'ingrédients unique, telle que la fiche
    // source la donnait. Deux schémas, donc deux familles — mélanger les deux
    // ferait passer les secondes pour des assemblages appauvris.
    case 'assemblages':    return 'assemblages';
    case 'classiques':     return 'classiques';
...
export const CATEGORIES = {
  ...
  classiques:     { label: "Les classiques",           ordre: 18 },
  traiteur:       { label: "Traiteur & salé",          ordre: 19 },
};

export const FAMILLES = {
  glacerie:          { label: 'Glacerie',            ordre: 1  },
  classiques:        { label: 'Les classiques',       ordre: 2  },
  assemblages:       { label: 'Assemblages',          ordre: 3  },
  biscuits:          { label: 'Biscuits',             ordre: 4  },
  gateaux_voyage:    { label: 'Gâteaux de voyage',   ordre: 5  },
  pates_fonds:       { label: 'Pâtes & Fonds',        ordre: 6  },
  cremes_mousses:    { label: 'Crèmes & Mousses',     ordre: 7  },
  chocolaterie:      { label: 'Chocolaterie',          ordre: 8  },
  caramels_pralines: { label: 'Caramels & Pralinés',  ordre: 9  },
  finitions_decors:  { label: 'Finitions & Décors',   ordre: 10  },
  fruits_conserves:  { label: 'Fruits & Conserves',   ordre: 11 },
  confiserie:        { label: 'Confiserie',            ordre: 12 },
  boissons:          { label: 'Boissons',              ordre: 13 },
  traiteur:          { label: 'Traiteur & salé',       ordre: 14 },
};
...
export const SOUS_CAT_LABELS = {
  ...
  boisson:               'Boissons',
  jus:                   'Jus & réductions',
  sirop:                 'Sirops & imbibages',
  // Les classiques
  feuilletage:           'Feuilletages',
  dessert_glace:         'Desserts glacés',
  dessert_dresse:        'Desserts dressés',
  // Traiteur & salé
  sale_proteine:         'Viandes & poissons',
  sauce_salee:           'Sauces & condiments',
  snacking:              'Snacking',
  garniture_salee:       'Garnitures salées',
  // Sous-catégories posées par l'import et jusqu'ici sans libellé : sans
  // entrée ici, FilterPanel retombe sur la clé technique (« decor_sucre »).
  genoise:               'Génoises & pains de Gênes',
  craquelin:             'Craquelins',
  barre_cereales:        'Barres de céréales',
  pate_amande:           "Pâtes d'amande",
  gianduja:              'Giandujas',
  beurre_compose:        'Beurres composés',
  creme_chiboust:        'Chibousts',
  ecume:                 'Écumes & émulsions',
  compote:               'Compotes',
  coulis:                'Coulis',
  noyau:                 'Noyaux & inserts fruits',
  confit_agrumes:        "Confits d'agrumes",
  pastillage:            'Pastillages',
  coloration:            'Colorations',
  decor_sucre:           'Décors sucre',
  sucre_decor:           'Sucres colorés',
  decor_chocolat:        'Décors chocolat',
};
