...
export function computeProductionPlan(plan, moulesMap) {
  ...
  if (moulesResolus.length === 0) {
    return {
      ...
      composants: couches.map(c => ({ ...c, volume_ml: 0, masse_au_moulage_g: 0, masse_a_preparer_g: 0 })),
  ...
  if (mode_calcul === 'par_couches') {
    ...
    // Densité moyenne pondérée par les pourcentages déclarés
    const densiteMoy = couches.reduce(
      (s, c) => s + ((c.pourcentage ?? 0) / 100) * (c.densite_g_ml ?? 1.0),
      0,
    );
    const masseTotaleBase = r1(volume_total_ml * densiteMoy);
    ...
    composants = couches.map(couche => {
      ...
      const masseAuMoulage  = couche.masse_g_override ?? r0(masseTotaleBase * pct / 100);
      const masseAPreparer  = r0(masseAuMoulage * (1 + perte_production_pct / 100));
      return {
        ...couche,
        volume_ml:          null,
