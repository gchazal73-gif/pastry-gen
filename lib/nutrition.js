...
function resolveNomIngredient(nom, table) {
  ...
  // 5. Repli insensible à la casse, aux accents et à la ligature.
  //    La table est écrite en capitale initiale (« Beurre », « Sucre semoule »,
  //    « Œufs entiers ») ; les recettes importées des fiches sources sont en
  //    minuscules. La recherche exacte les manquait toutes : 2 184 recettes sur
  //    3 896 ressortaient à 0 kcal, sur des ingrédients aussi courants que le
  //    beurre, le sucre et la farine.
  const cle = normaliseCle(nom);
  const parCle = indexInsensible(table);
  if (parCle.has(cle)) return parCle.get(cle);

  const cleSansParenthese = normaliseCle(sansParenthese);
  if (cleSansParenthese !== cle && parCle.has(cleSansParenthese)) {
    return parCle.get(cleSansParenthese);
  }

  return nom;
}

function normaliseCle(s) {
  return (s || '')
    .toLowerCase()
...
// Index construit une fois par table. Les tests injectent leurs propres tables,
// d'où le cache par identité plutôt qu'une constante de module.
const _indexParTable = new WeakMap();

function indexInsensible(table) {
  let idx = _indexParTable.get(table);
  if (idx) return idx;
  idx = new Map();
  // Premier arrivé gagne : l'ordre de déclaration de la table fait autorité
  // quand deux libellés se normalisent pareil.
  for (const cle of Object.keys(table)) {
    const k = normaliseCle(cle);
    if (!idx.has(k)) idx.set(k, cle);
  }
  _indexParTable.set(table, idx);
  return idx;
}
