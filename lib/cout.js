...
//
// Le référentiel ne couvre que 58 ingrédients : la résolution passe ensuite par
// `ingredients-metier-familles.js`, qui relie les synonymes (« Sucre » ->
// « Sucre semoule ») puis, à défaut, applique un prix de famille moyen. Chaque
// ligne remonte sa `source` — un prix de famille est une approximation, et la
// fiche doit le dire plutôt que de la fondre dans le total.
//
// Sans cela, un ingrédient inconnu comptait pour zéro : une recette paraissait
// d'autant moins chère qu'on en connaissait moins les ingrédients.
// Formats d'entrée acceptés : lignes du générateur V3 ({ ingredient, g }) ou
//   recettes bibliothèque ({ nom, g })

import { resoudrePrix } from './ingredients-metier-familles.js';
...
export function calculerCoutRecette(lignes, ingredientsMetier = {}) {
  ...
  const ingredients_prix_approche = [];
  let   masse_prix_approche_g     = 0;
  const prix_obsolete          = [];
  ...
  for (const ligne of lignes) {
    ...
    // Le référentiel d'abord (y compris les overrides de la mercuriale), puis
    // synonyme, puis famille.
    let m = lookupMetier(nom, ingredientsMetier);
    let source = m ? 'referentiel' : null;
    if (!m || m.prix_eur_par_g == null) {
      const res = resoudrePrix(nom, ingredientsMetier);
      if (res) { m = res.fiche; source = res.source; }
    }

    if (!m || m.prix_eur_par_g == null) {
      ingredients_sans_prix.push(nom);
      cout_par_ligne.push({ ingredient: nom, masse_g, prix_eur_par_g: null, cout_eur: 0, part_pct: null, source: 'aucun' });
      continue;
    }
    ...
    if (source === 'famille') {
      masse_prix_approche_g += masse_g;
      if (!ingredients_prix_approche.includes(nom)) ingredients_prix_approche.push(nom);
    }
  ...
  return {
    ...
    // Prix de famille : une moyenne du référentiel, pas le prix de l'article.
    // Le taux dit quelle part du coût repose dessus.
    ingredients_prix_approche,
    part_prix_approche_pct: masse_avec_prix_g > 0
      ? Math.round((masse_prix_approche_g / masse_avec_prix_g) * 1000) / 10
      : 0,
    prix_obsolete,
