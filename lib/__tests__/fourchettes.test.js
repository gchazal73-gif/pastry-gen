...
import { FOURCHETTES_TEXTURE, TEXTURE_VERS_FOURCHETTE, resoudreFourchettes, verifierFourchettes } from '../fourchettes.js';
...
describe('verifierFourchettes — statuts ok / attention / hors / inconnu', () => {
  ...
  // Ce test affirmait l'inverse : le retour vide était le comportement attendu.
  // C'est précisément ce silence qui a laissé le générateur tourner sans
  // qu'aucune fourchette ne lui soit jamais opposée.
  it('lève une erreur explicite pour une texture inconnue', () => {
    expect(() => verifierFourchettes({}, 'texture_inexistante'))
      .toThrow(/Aucune fourchette pour la texture/);
  });
});

// ── 2 bis : correspondance TEMPLATES ↔ FOURCHETTES ───────────────────────────
// Test de dérive : si une texture est ajoutée à TEMPLATES sans fourchette ni
// correspondance, elle arriverait muette jusqu'à l'écran. Ici elle casse.
describe('TEXTURE_VERS_FOURCHETTE — toute texture générable a une cible', () => {
  it('tous les IDs de TEMPLATES résolvent vers une fourchette existante', async () => {
    const { TEMPLATES } = await import('../templates.js');
    const manquantes = Object.keys(TEMPLATES).filter(id => !resoudreFourchettes(id));
    expect(manquantes).toEqual([]);
  });

  it('chaque cible de TEXTURE_VERS_FOURCHETTE existe dans FOURCHETTES_TEXTURE', () => {
    const orphelines = Object.entries(TEXTURE_VERS_FOURCHETTE)
      .filter(([, cle]) => !FOURCHETTES_TEXTURE[cle])
      .map(([id, cle]) => `${id} → ${cle}`);
    expect(orphelines).toEqual([]);
  });

  it('une texture générable produit un rapport non vide', () => {
    const rapport = verifierFourchettes(
      { es: 36, mg: 12, sucres: 20, fibres: 5, lactose: 2, h2o_libre: 50, ph: 3.5 },
      'mousse_bconcept_fruit',
    );
    expect(rapport.length).toBeGreaterThan(0);
    expect(rapport.every(r => r.statut === 'ok')).toBe(true);
  });
});
