...
export const TEMPLATES = {
  ...
  ganache_montee_noir: {
    ...
    lines: [
      ...
      { role: "sucrant_anti_cristal", label: "Sucrant anti-cristal", pct: 5,
        getIngredient: (_p, c) => {
          if (c.bien_etre) return { nom: "Sirop d'érable", note: "Remplace le glucose DE38 — rôle anti-cristal similaire, 34 % d'eau intrinsèque compensée par la réduction de l'eau d'ajustement" };
          if (c.igbas)     return { nom: "Oligofructose", note: "Substitut IG bas — même rôle anti-cristallisation que le glucose" };
          return { nom: "Glucose DE38", note: "Limite la cristallisation du sucre au repos" };
        }
      },
      { role: "sucre_sec_be", label: "Sucre de fleur de coco", pct: 2,
        getIngredient: () => ({ nom: "Sucre de coco", note: "Apporte la douceur sèche que le sirop d'érable ne peut pas remplir seul — maintient le taux de matière sèche proche du classique" }),
        actif: (_p, c) => !!c.bien_etre
      },
      { role: "eau", label: "Eau d'ajustement", pct: 3,
        getIngredient: () => ({ nom: "Eau" }),
        pctOverride: (_p, c) => c.bien_etre ? 1 : 3
      }
    ],
    ...
    process: (p, c) => {
      ...
      const sucrant = c.bien_etre ? "le sirop d'érable et le sucre de coco" : "le glucose";
      const repos   = c.bien_etre ? "14–16" : "12";
      const reposNote = c.bien_etre ? " — le sirop d'érable cristallise plus lentement que le glucose, ne pas abréger le repos." : ".";
      return [
        `Chauffer ${cremeChaude} avec ${sucrant} à 70–75 °C.`,
        ...
        `Filmer au contact. Repos minimum ${repos} h à +4 °C${reposNote}`,
  ...
  ganache_montee_lait: {
    ...
    lines: [
      ...
      { role: "eau", label: "Eau d'ajustement", pct: 7,
        getIngredient: () => ({ nom: "Eau" }),
        pctOverride: (_p, c) => c.bien_etre ? 5 : 7
      }
    ],
    ...
    process: (p, c) => {
      ...
      return [
        ...
        `Ajouter ${cremeFroide} d'un coup, mixer 30 s.`,
        `Filmer au contact. Repos minimum ${repos} h à +4 °C${c.bien_etre ? " — le sirop d'érable cristallise plus lentement que le glucose, ne pas abréger le repos." : "."}`,
  ...
  ganache_montee_blanc: {
    ...
    lines: [
      ...
      { role: "eau", label: "Eau d'ajustement", pct: 5,
        getIngredient: () => ({ nom: "Eau" }),
        pctOverride: (_p, c) => c.bien_etre ? 3 : 5
      }
    ],
    ...
    process: (p, c) => {
      ...
      const repos   = c.bien_etre ? "14–16" : "12";
      return [
        `Chauffer ${cremeChaude} avec ${sucrant} à 70 °C (ne pas dépasser — le blanc est sensible à la surcuisson).`,
        `Verser en 3 fois sur ${choc} fondu à ${tempFusion}. Mixer au mixeur plongeant en cercles serrés, sans incorporer d'air.`,
        `Ajouter ${cremeFroide} d'un coup, mixer 30 s.`,
        `Filmer au contact. Repos minimum ${repos} h à +4 °C${c.bien_etre ? " — le sirop d'érable cristallise plus lentement que le glucose, ne pas abréger le repos." : " (critique pour le blanc qui cristallise plus lentement que le noir)."}`,
  ...
  ganache_montee_fruit: {
    ...
    lines: [
      ...
      { role: "sucrant_anti_cristal", label: "Sucrant anti-cristal", pct: 4,
        getIngredient: (_p, c) => {
          if (c.bien_etre) return { nom: "Sirop d'érable", note: "Remplace le glucose DE38 — rôle anti-cristal similaire ; 34 % d'eau intrinsèque compensée par la réduction de l'eau d'ajustement" };
          if (c.igbas)     return { nom: "Oligofructose", note: "Substitut IG bas" };
          return { nom: "Glucose DE38", note: "Limite la cristallisation — réduit aussi l'acidité perçue" };
        }
      },
      { role: "sucre_sec_be", label: "Sucre de fleur de coco", pct: 2,
        getIngredient: () => ({ nom: "Sucre de coco", note: "Apporte la douceur sèche que le sirop d'érable ne peut pas remplir seul" }),
        actif: (_p, c) => !!c.bien_etre
      },
      { role: "gelifiant", label: "Gélifiant", pct: 1,
        getIngredient: (_p, c) => c.vegan
          ? { nom: "Pectine X58", note: "Mélanger à sec avec le sucrant avant dispersion dans la purée froide — activer à 65 °C" }
      ...
      { role: "eau", label: "Eau d'ajustement", pct: 9,
        getIngredient: () => ({ nom: "Eau" }),
        pctOverride: (_p, c) => c.bien_etre ? 7 : 9
      }
    ],
    ...
    process: (p, c) => {
      ...
      if (!c.vegan) {
        ...
        steps.push(`Chauffer ${nomPuree} avec ${sucrant} à 60 °C. Incorporer la gélatine essorée hors du feu, mixer pour dissoudre.`);
      } else {
        steps.push(`Mélanger à sec la pectine X58 avec ${sucrant}. Disperser dans ${nomPuree} froide, chauffer à 65 °C en fouettant pour activer la pectine.`);
      }
      ...
      steps.push(`Filmer au contact. Repos minimum ${repos} h à +4 °C${c.bien_etre ? " — le sirop d'érable cristallise plus lentement que le glucose, ne pas abréger le repos." : "."}`);
  ...
  ganache_montee_praline: {
    ...
    lines: [
      ...
      { role: "eau", label: "Eau d'ajustement", pct: 4,
        getIngredient: () => ({ nom: "Eau" }),
        pctOverride: (_p, c) => c.bien_etre ? 2 : 4
      }
    ],
  ...
  sable_breton: {
    ...
    lines: [
      { role: "mg", label: "Beurre demi-sel", pct: 40,
        ...
        pctOverride: (_p, c) => c.bien_etre ? 24 : 40
      },
      { role: "puree_amande_be", label: "Purée d'amande (bien-être)", pct: 16,
        getIngredient: () => ({ nom: "Purée d'amande", note: "À 20 °C pommade — mélanger avec le beurre ramolli avant de sabler. Apporte acides gras insaturés + 12 % fibres" }),
        actif: (_p, c) => !!c.bien_etre
      },
    ...
    process: (_p, c) => {
      ...
      const sucre     = (c.igbas || c.bien_etre) ? "le sucre de coco" : "le sucre semoule";
      const epais     = c.format !== 'fin';
      const epaisseur = epais ? "8" : "4";
      const temp      = epais ? "175" : "180";
      const duree     = epais ? "16–18" : "10–12";
      const repos     = c.bien_etre ? "45" : "30";
      return [
        c.bien_etre
          ? `Mélanger à la spatule le beurre pommade avec la purée d'amande à 20 °C jusqu'à homogénéité — ne pas battre au robot, la purée d'amande est oléagineuse et chauffe vite.`
          : `Sortir ${beurre} 1 h avant pour qu'il soit souple sans être fondu.`,
        `Dans la cuve du batteur muni de la feuille, sabler ${c.bien_etre ? "le mélange beurre-purée d'amande" : beurre} avec ${farine} et la levure jusqu'à obtention d'une texture sableuse grossière.`,
        `Ajouter ${sucre} et ${liant}. Mélanger à vitesse lente jusqu'à incorporation — ne pas travailler la pâte au-delà.`,
        `Étaler entre deux feuilles de papier sulfurisé à ${epaisseur} mm. Réserver au réfrigérateur ${repos} min minimum${c.bien_etre ? " — la purée d'amande ramollit la pâte, un froid plus long est nécessaire pour tenir à l'étalage" : " (la pâte est fragile à chaud)"}.`,
  ...
  pate_sucree: {
    ...
    lines: [
      { role: "mg", label: "Beurre pommade", pct: 28,
        ...
        pctOverride: (_p, c) => c.bien_etre ? 17 : 28
      },
      { role: "puree_amande_be", label: "Purée d'amande (bien-être)", pct: 11,
        getIngredient: () => ({ nom: "Purée d'amande", note: "À 20 °C pommade — crémer avec le beurre avant d'ajouter le sucrant. Apporte acides gras insaturés + 12 % fibres" }),
        actif: (_p, c) => !!c.bien_etre
      },
      { role: "sucrant", label: "Sucre glace", pct: 15,
        getIngredient: (_p, c) => (c.igbas || c.bien_etre)
          ? { nom: "Sucre de coco (mixé fin)", note: "Tamiser finement — rôle similaire au sucre glace pour limiter le réseau glutineux" }
          : { nom: "Sucre glace", note: "Tamisé — le sucre glace évite le développement du réseau glutineux" }
      },
    ...
    process: (_p, c) => {
      ...
      const farine   = c.gluten ? "le mix sans gluten" : "la farine T55";
      const sucrant  = (c.igbas || c.bien_etre) ? "le sucre de coco tamisé" : "le sucre glace";
      const thin     = c.format === 'tartelette';
      const epais    = thin ? "2" : "3";
      const temp     = thin ? "165" : "160";
      const duree    = thin ? "10–12" : "14–16";
      const repos    = thin ? (c.bien_etre ? "30" : "20") : (c.bien_etre ? "45" : "30");
      return [
        c.bien_etre
          ? `Mélanger à la spatule ${beurre} avec la purée d'amande à 20 °C jusqu'à homogénéité, puis crémer avec ${sucrant} à la feuille.`
          : `Crémer ${beurre} avec ${sucrant} à la feuille jusqu'à blanchiment léger.`,
        ...
        `Former un disque, filmer et réserver au réfrigérateur ${repos} min minimum${c.bien_etre ? " — la purée d'amande ramollit la pâte, ne pas raccourcir le repos" : ""}.`,
  ...
  streusel: {
    ...
    lines: [
      { role: "mg", label: "Beurre froid", pct: 25,
        ...
        pctOverride: (_p, c) => c.bien_etre ? 15 : 25
      },
      { role: "puree_amande_be", label: "Purée d'amande (bien-être)", pct: 10,
        getIngredient: () => ({ nom: "Purée d'amande", note: "Froide (+4 °C) — couper en petits cubes et remettre au congélateur 15 min avant usage pour faciliter le sablage" }),
        actif: (_p, c) => !!c.bien_etre
      },
      ...
      { role: "sucrant", label: "Cassonade", pct: 25,
        getIngredient: (_p, c) => (c.igbas || c.bien_etre)
          ? { nom: "Sucre de coco", note: "Notes caramélisées, IG 35 — les cristaux maintiennent le craquant" }
    ...
    process: (_p, c) => {
      ...
      const sucre   = (c.igbas || c.bien_etre) ? "le sucre de coco" : "la cassonade";
      const farine  = c.gluten ? "la farine de riz" : "la farine T55";
      const mg      = c.bien_etre ? `${beurre} et la purée d'amande froide` : beurre;
      ...
      return [
        c.bien_etre
          ? `Mettre la purée d'amande 15 min au congélateur pour la raffermir. Dans un cul-de-poule, réunir ${farine}, ${sucre} et la poudre d'amande.`
          : `Dans un cul-de-poule, réunir ${farine}, ${sucre} et la poudre d'amande.`,
        grossier
          ? `Incorporer ${mg} du bout des doigts en sablant rapidement jusqu'à l'obtention de miettes ${taille}. Ne pas chercher une pâte homogène — les grosses miettes donnent la texture crumble.`
          : `Incorporer ${mg} en sablant, puis mixer 2–3 pulsations courtes au robot coupe pour obtenir des miettes ${taille}. Arrêter avant que la pâte ne s'amalgame.`,
  ...
  biscuit_cuillere: {
    ...
    lines: [
      ...
      { role: "sucrant_jaunes", label: "Sucrant (jaunes)", pct: 10,
        getIngredient: (_p, c) => (c.igbas || c.bien_etre)
          ? { nom: "Sucre de coco", note: "Blanchir les jaunes jusqu'à ruban — notes caramélisées légères" }
  ...
  dacquoise: {
    ...
    process: (p, c) => {
      ...
      const sucrant  = (c.igbas || c.bien_etre) ? "le sucre de coco mixé finement" : "le sucre glace tamisé";
      ...
      return [
        ...
        `Monter ${blancs} en neige souple. Incorporer ${sucrant} en 3 fois pour obtenir une meringue ferme et brillante.`,
        ...
        `Saupoudrer légèrement de ${(c.igbas || c.bien_etre) ? "sucre de coco en poudre fine" : "sucre glace"} — il fond et forme la croûte craquante caractéristique.`,
  ...
  financier: {
    ...
    lines: [
      { role: "mg", label: "Beurre noisette", pct: 35,
        ...
        pctOverride: (_p, c) => c.bien_etre ? 20 : 35
      },
      { role: "mg_be", label: "Purée d'amande (bien-être)", pct: 15,
        getIngredient: () => ({ nom: "Purée d'amande", note: "Incorporer à 20 °C après la MG chaude — ne pas chauffer. Apporte 55 % de MG insaturée + 12 % fibres" }),
        actif: (_p, c) => !!c.bien_etre
      },
      ...
      { role: "sucrant", label: "Sucre glace", pct: 25,
        getIngredient: (_p, c) => (c.igbas || c.bien_etre)
          ? { nom: "Sucre de coco (mixé fin)", note: "Tamiser finement — légères notes caramélisées, IG plus bas" }
    ...
    process: (p, c) => {
      ...
      const sucrant   = (c.igbas || c.bien_etre) ? "le sucre de coco" : "le sucre glace";
      const mini      = c.format === 'mini';
      const temp      = mini ? "200" : "180";
      const duree     = mini ? "8" : "12";
      return [
        (c.vegan || c.lactose)
          ? `Chauffer l'huile de noisette à 40 °C${c.bien_etre ? '. Incorporer la purée d\'amande à 20 °C en mélangeant — ne pas chauffer la purée.' : '.'}`
          : c.bien_etre
            ? `Réaliser le beurre noisette sur la masse réduite. Passer au chinois, laisser refroidir à 40 °C. Incorporer la purée d'amande à 20 °C en fouettant — ne pas réchauffer la purée.`
        ...
        `Mélanger ${poudre} tamisée avec ${sucrant} et la farine.`,
        `Incorporer ${blancs} (non montés) en fouettant jusqu'à pâte homogène.`,
        `Verser ${beurre} tiède${c.bien_etre ? ' puis la purée d\'amande' : ''} en filet en mélangeant. La pâte doit être lisse et brillante.`,
  ...
  madeleine: {
    ...
    lines: [
      ...
      { role: "mg_be", label: "Purée d'amande (bien-être)", pct: 15,
        getIngredient: () => ({ nom: "Purée d'amande", note: "À 20 °C — incorporer après le beurre fondu refroidi. Apporte MG insaturée et liant naturel" }),
        actif: (_p, c) => !!c.bien_etre
      },
      ...
      { role: "parfum_miel", label: "Miel / sirop", pct: 2,
        getIngredient: (_p, c) => c.vegan
          ? { nom: "Sirop d'agave", note: "Remplace le miel — même rôle hygroscopique pour la conservation" }
          : c.bien_etre
            ? { nom: "Sirop d'érable", note: "Remplace le miel — même rôle hygroscopique, notes plus complexes" }
    ...
    process: (_p, c) => {
      ...
      const sucre   = (c.igbas || c.bien_etre) ? "le sucre de coco" : "le sucre semoule";
      const farine  = c.gluten ? "le mix sans gluten" : "la farine T55";
      const miel    = c.vegan ? "le sirop d'agave" : c.bien_etre ? "le sirop d'érable" : "le miel";
      return [
        c.bien_etre
          ? `Faire fondre ${beurre} et le faire légèrement brunir. Refroidir à 40 °C. Incorporer la purée d'amande à la spatule jusqu'à mélange homogène.`
  ...
  cake_voyage: {
    ...
    lines: [
      ...
      { role: "mg_be", label: "Purée d'amande (bien-être)", pct: 10,
        getIngredient: () => ({ nom: "Purée d'amande", note: "À 20 °C pommade — mélanger avec le beurre avant de crémer. Apporte MG insaturée et moelleux naturel" }),
        actif: (_p, c) => !!c.bien_etre
      },
    ...
    process: (_p, c) => {
      const beurre = (c.vegan || c.lactose) ? "le beurre végétal" : "le beurre pommade";
      const mg     = c.bien_etre ? `${beurre} et la purée d'amande (mélangés ensemble à la spatule)` : beurre;
      const liant  = c.vegan ? "l'aquafaba" : "les œufs";
      const farine = c.gluten ? "le mix sans gluten" : "la farine T55";
      ...
      return [
        `Crémer ${mg} avec ${sucre} au batteur (feuille) à vitesse moyenne, 3–4 min, jusqu'à mélange pâle et aéré.`,
